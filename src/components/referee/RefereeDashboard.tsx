...
export const RefereeDashboard: React.FC = () => {
  ...
  // Helper: update last active instantly
  const updateLastActiveNow = async () => {
    if (!currentRefereeId) return;
    try {
      await updateDoc(doc(db, "referees", currentRefereeId), { lastActive: serverTimestamp() });
    } catch (err) {
      console.error("⚠️ Failed to update lastActive:", err);
    }
  };
  ...
  useEffect(() => {
    if (!currentRefereeId) return;
    const refereeDoc = doc(db, "referees", currentRefereeId);

    const updateLastActive = async () => {
      try {
        await updateDoc(refereeDoc, { lastActive: serverTimestamp() });
      } catch (error) {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      const grouped = raw.reduce((acc: any, r) => {
        ...
        if (!acc[key])
          acc[key] = {
            matchId: key,
            matchDisplay: `${r.homeTeam} vs ${r.awayTeam}`,
            reports: [],
            auditTrail: [],
          };
  ...
  const pending = appointments.filter(
    (a) => !a.responses?.[a._role]?.status || a.responses[a._role].status === "pending"
  ).length;
    ...
    return (
      <div className="p-4 sm:p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button variant="outline" size="sm" onClick={() => setViewProfile(false)}>
            Back
          </Button>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="hidden sm:flex justify-between items-center bg-white p-6 rounded-xl shadow-sm">
            ...
            <div className="flex items-center gap-4">
              ...
              <img
                ...
                className="w-12 h-12 rounded-full border-2 border-emerald-500 cursor-pointer"
                alt="profile"
              />
          ...
          {loading ? (
            ...
            appointments.map((apt) => (
              <Card key={apt.id} className="p-4 shadow-sm mb-4">
          ...
          {/* Reports */}
          <h3 className="text-xl sm:text-2xl font-bold mt-6 mb-3">My Reports</h3>
          {reports.length === 0 ? (
              ...
              {reports.map((group: any) => (
                ...
                >
                  <div className="flex justify-between mb-2">
                    <div className="flex gap-1 flex-wrap">
                      {group.reports.map((r: any) => (
                        <Badge
                          key={r.id}
                          variant={r.reviewed ? "success" : "warning"}
                          className="text-xs"
                        >
                          {r.type.replace("_", " ")}
                        </Badge>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {group.auditTrail.length} audit actions
                  </p>
                </Card>
              ))}
            </div>
