...
export const RoleSelector: React.FC<RoleSelectorProps> = ({ onSelectRole }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <h2 className="text-3xl font-bold text-center mb-8 text-gray-900">Select Your Role</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {roles.map(({ role, title, desc, icon }) => (
          <Card key={role} onClick={() => onSelectRole(role)} className="text-center hover:border-emerald-600 border-2 border-transparent">
